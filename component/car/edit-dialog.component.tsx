...
export const EditDialog = memo(
    ({ open, onClose, onSave, data }: EditCarDialogProps) => {
        ...
        const onSubmit: SubmitHandler<UpdateCarModel> = async (formData) => {
            onSave({
                ...formData,
                hasDualBrake: formData.hasDualBrake?.toString() === 'true',
                isActive: formData.isActive?.toString() === 'true'
            });
        };
        ...
        const checkChassisNumber = async (value: string | undefined) => {
            if (!value) return true;
            const res = await debounceUniqueChassisNumber(value, Number(data?.car_id));
            return res || 'Số khung đã tồn tại';
        };

        const checkEngineNumber = async (value: string | undefined) => {
            if (!value) return true;
            const res = await debounceUniqueEngineNumber(value, Number(data?.car_id));
